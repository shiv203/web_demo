(function(window, document) {
    ...
    handwriting.Canvas = function(cvs, lineWidth) {
        ...
        this.recognize2 = handwriting.recognize2;
    };
    /**
            ...
            if (this.readyState === 4) {
                switch (this.status) {
                    case 200:
                        ...
                        var results;
                        if (response.length === 1) callback(undefined,undefined, new Error(response[0]));
                        ...
                        document.getElementById("google-online").innerHTML=results;

                        callback(trace,results, undefined);
                        
                        break;
                    case 403:
                        callback(undefined,undefined, new Error("access denied"));
                        break;
                    case 503:
                        callback(undefined,undefined, new Error("can't connect to recognition server"));
                        break;
                }
    ...
    handwriting.recognize2 = function(trace, options, callback) {
        if (handwriting.Canvas && this instanceof handwriting.Canvas) {
            trace = this.trace;
            options = this.options;
            callback = this.callback;
                ...
                switch (this.status) {
                    case 200:
                        ...
                        //if (!!options.numOfReturn) {
                          //  results = results.slice(0, options.numOfReturn);
                        //}
                        document.getElementById("vision-online").innerHTML=response.out;
        ...
        xhr.open("POST", "http://shiva-8700:5000/onlrec");
        xhr.setRequestHeader("content-type", "application/json");
        xhr.send(data);
    };
